...
const isSuccess = ref(localStorage.getItem('isSuccessOTP') === 'true');
...
const submitVerification = async () => {
    ...
    try {
        ...
        if (response.status === 200) {
            localStorage.setItem('isSuccessOTP', 'true');
...
const resendOtp = async () => {
    ...
    try {
        const response = await customFetch.post('/auth/generate-otp-code', {
...
const redirectToAdditionalInfo = () => {
    router.push('/businessDataForm'); // Redirect ke dashboard atau halaman lain
};
